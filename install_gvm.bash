...
function print_help() {
    echo 'Greenbone Vulnerability Manager - Installation Script'
    echo ''
    echo 'Variables below can be modified.'
    echo ''
    ...
    echo '  GVM_VERSION        : GVM version to install. (default = 20.08)'
    echo '  GVM_ADMIN_PWD      : Initial admin password. (default = admin)'
    echo '  GVM_TIMEZONE       : OS Timezone (default = UTC)   '
    echo ''
}
...
export GVM_VERSION="${GVM_VERSION:-20.08}"
export GVM_ADMIN_PWD="${GVM_ADMIN_PWD:-admin}"
export GVM_TIMEZONE="${GVM_TIMEZONE:-UTC}"

require GVM_INSTALL_PREFIX
require GVM_VERSION
require GVM_ADMIN_PWD
require GVM_TIMEZONE
...
function update_timezone() {
    set -e
    timedatectl set-timezone "$GVM_TIMEZONE"
}

log -i "Setting OS Timezone"
exec_as root update_timezone GVM_TIMEZONE
log -i "Update system"
...
log -w "Feed sync will take some additional time to be done"
log -i "If necessary, retry it (one at a time) :"
log -i "Update NVT : greenbone-nvt-sync"
log -i "Update SCAP : greenbone-feed-sync --type SCAP"
log -i "Update CERT : greenbone-feed-sync --type CERT"
log -i "Update CERT : greenbone-feed-sync --type GVMD_DATA"